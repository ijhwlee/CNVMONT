...
const createPzLib = ({ v3d=null, appInstance=null }) => {
    ...
    return {
        ...
        transformEulerV3dToBlenderShortest, RotationInterface, MediaHTML5, AudioWebAudio,
...
this.procedures["update map"] = update_map;

var map_half_size_x, change, map_half_size_y, cursor_x, footsteps, cursor_y, distance, sounds, footsteps_sound;
...
// getObjectDimension puzzle

_pGlob.box3Tmp = new v3d.Box3();

function getObjectDimension(objSelector, coord, isWorldSpace) {

    const box = _pGlob.box3Tmp.makeEmpty();
    ...
    if (isWorldSpace) {
        ...
        if (!obj) {
            console.error("get object dimension puzzle: Invalid object");
            return;
        }
        ...
        // COMPAT: < Verge3D 4.2 (new puzzles, old engine)
        if (box.scale)
            box.scale(obj.scale);

    }

    const size = box.getSize(_pGlob.vec3Tmp);

    const coordSystem = PzLib.getSceneCoordSystem();
    PzLib.transformCoordsSpace(size, 'Y_UP_RIGHT', coordSystem, true);
    ...
    case 'XYZ':
        return size.toArray();
    default:
        console.error("get object dimension puzzle: Wrong coords");
        return;
    }
};
...
function update_map() {
    cursor_x = (getObjTransform('Camera', false, 'position', 'x') - -20) / (20 - -20) * (map_half_size_x - -map_half_size_x) + -map_half_size_x;
    cursor_y = (getObjTransform('Camera', false, 'position', 'y') - -25) / (22 - -25) * (map_half_size_y - -map_half_size_y) + -map_half_size_y;
    setObjTransform('cursor', false, 'position', [cursor_x, cursor_y, ''], false);
    setObjTransform('cursor', false, 'rotation', ['', '', getObjTransform('Camera', false, 'rotation', 'z')], false);
}
...
map_half_size_x = getObjectDimension('map', 'X', false) / 2;
map_half_size_y = getObjectDimension('map', 'Y', false) / 2;

update_map();

whenMoved('Camera', 0.01, function() {}, function() {
    update_map();
    distance = getDistanceBetweenObjects('Camera', 'ReflectionCubemap.001');
    volume(loadMedia_HTML5('sounds/env_sound.mp3'), Math.min(Math.max((distance - 6.5) / (8 - 6.5) * (0.4 - 0.2) + 0.2, 0.2), 0.4));
    if (toFixedPoint(getObjTransform('Camera', false, 'position', 'z'), 2) <= 1.75) {
        if (footsteps_sound != loadMedia_WebAudio('sounds/footstep_ext.mp3')) {
            footsteps_sound = loadMedia_WebAudio('sounds/footstep_ext.mp3');
            change = true;
        }
    } else {
        if (footsteps_sound != loadMedia_WebAudio('sounds/footstep_int.mp3')) {
            footsteps_sound = loadMedia_WebAudio('sounds/footstep_int.mp3');
            change = true;
        }
    }
    if (!footsteps || change) {
        volume(loadMedia_WebAudio('sounds/footstep_ext.mp3'), 0);
        volume(loadMedia_WebAudio('sounds/footstep_int.mp3'), 0);
        volume(footsteps_sound, 0.2);
        footsteps = true;
        change = false;
    }
}, function() {
...
footsteps = false;
sounds = false;
volume(loadMedia_HTML5('sounds/env_sound.mp3'), 0.4);
volume(loadMedia_WebAudio('sounds/footstep_ext.mp3'), 0.2);
volume(loadMedia_WebAudio('sounds/footstep_int.mp3'), 0.2);
eventHTMLElem('click', ['WINDOW'], false, function(event) {
    if (!sounds) {
        sounds = true;
        playSound(loadMedia_HTML5('sounds/env_sound.mp3'), true);
        footsteps_sound = loadMedia_WebAudio('sounds/footstep_ext.mp3');
        playSound(loadMedia_WebAudio('sounds/footstep_ext.mp3'), true);
        playSound(loadMedia_WebAudio('sounds/footstep_int.mp3'), true);
    }
});
